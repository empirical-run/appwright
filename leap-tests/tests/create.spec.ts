...
test("Create new wallet by entering the recovery phrase", async ({ client, saveVideo }) => {
...
test("Create new wallet by entering invalid word throws an error", async ({ client, saveVideo }) => {
    const onboardingPage = new OnboardingPage(client);
    await onboardingPage.enterPin();
