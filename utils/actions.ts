...
export async function click({client, xpath}: {client: Client; xpath: string}){
...
export async function waitAndClick(
    ...
    try {
        await waitUntil(client, async () => {
            ...
            return await client.isElementDisplayed(element["element-6066-11e4-a52e-4f735466cecf"]);
        }, {
            timeout: timeout,
            interval: interval,
        ...
        throw new Error(`Failed to click on the element with XPath "${xpath}": ${error.message}`);
    }
}
...
export async function isElementVisibleWithinTimeout(
    ...
    try {
        const isVisible = await waitUntil(
            ...
            async () => {
                try {
                    ...
                    console.log(`Error while checking visibility of element with XPath "${xpath}": ${error.message}`);
                    return false;
                }
            },
            {
        ...
        console.log(`Error or timeout occurred while waiting for element: ${error.message}`);
        return false;
    }
}
...
export async function setValue(client: Client, xpath: string, value: string) {
    const element = await client.findElement('xpath', xpath);
    await client.elementSendKeys(element["element-6066-11e4-a52e-4f735466cecf"], value);
}
